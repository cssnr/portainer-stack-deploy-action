...
function getEnv(inputs, stack) {
  ...
  if (inputs.merge_env && stack?.Env?.length) {
    ...
    const current = Object.fromEntries(stack.Env.map(({ name, value }) => [name, value]))
...
async function addSummary(inputs, stack) {
  ...
  core.summary.addTable([
    ...
    [{ data: 'Created' }, { data: new Date(stack.CreationDate * 1000).toLocaleString() }],
    [
      { data: 'Updated' },
      {
        data: stack.UpdateDate ? new Date(stack.UpdateDate * 1000).toLocaleString() : '-',
      },
    ],
